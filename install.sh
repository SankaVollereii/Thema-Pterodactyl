...
check_token() {
  ...
  if [ "$USER_TOKEN" = "sanka" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di Sanka Vollerei${NC}"
    echo -e "${YELLOW}TELEGRAM : @OnlySankaaa${NC}"
    echo -e "${YELLOW}WHATSAPP : +6282375165100${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 15K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© Sanka Vollereii${NC}"
    exit 1
  fi
  clear
}
